...
export default function AgentMonitor({
  ...
  const NUDGE_PROMPTS = [
    "Continue building. Write the next missing file as a complete code block using its canonical path. Do not summarize or describe — write the actual code now. Use DoBetter Design System light theme: --bg #F4F6FB, --sidebar #FFFFFF, --accent #5B6EF5, Syne headings, DM Sans body.",
    "The preview is showing minimal or blank content. This means files are missing or contain placeholder data. Re-write ALL 8 files starting with index.html. Requirements: LIGHT theme (--bg: #F4F6FB, --sidebar: #FFFFFF, --accent: #5B6EF5), sidebar with 5 routes, 4 KPI stat cards (UPPERCASE label + Syne value + trend arrow + sparkline), SVG bar chart, data table with badge cells. NO Alice/Bob/User1/Item1/Lorem ipsum — use realistic domain-specific data with specific (non-round) values like 47,291 or $2,847.63.",
    "Output is incomplete. Write the remaining missing files now. Start with ```javascript:src/js/components.js — include createSidebar() with 'MAIN MENU' section label + icon nav items with data-nav-item + user footer, createNavbar(), createStatCard(), createChart(), createModal(), createDataTable(), renderDashboard(), and a renderXxx() function for EVERY sidebar route. Then write src/js/app.js with tailwind.config at the top using DoBetter light tokens.",
    "The project needs premium visual quality matching .github/agents/dobetter-dashboard.jsx. Rebuild src/css/styles.css with the complete DoBetter Design System light token baseline (:root { --bg: #F4F6FB; --sidebar: #FFFFFF; --card: #FFFFFF; --border: #E8ECF4; --text: #1A1D23; --sub: #6B7280; --accent: #5B6EF5; --al: #EEF0FE; --shadow: 0 1px 3px rgba(0,0,0,.06), 0 2px 10px rgba(0,0,0,.04); }). Rebuild src/css/components.css with 12px radius cards, tinted-background badges (never solid fills), sidebar active states (--al bg + --accent text), hover transitions. Then rebuild src/js/state.js with 12+ realistic domain-specific data records (no placeholder names or round numbers).",
    "Complete the build now. Write src/js/app.js immediately. It MUST start with: tailwind.config = { theme: { extend: { colors: { accent: { DEFAULT: '#5B6EF5', dark: '#6366F1', muted: 'rgba(91,110,245,0.12)', light: '#EEF0FE' }, success: { DEFAULT: '#22C55E', light: '#DCFCE7' }, warning: { DEFAULT: '#F59E0B', light: '#FEF3C7' }, danger: { DEFAULT: '#EF4444', light: '#FEE2E2' } }, fontFamily: { sans: ['DM Sans', 'sans-serif'], heading: ['Syne', 'sans-serif'] }, borderRadius: { md: '12px' } } } }; Then init() function registering ALL routes (one per sidebar item), rendering layout shell, calling handleRoute(). End with document.addEventListener('DOMContentLoaded', init). After writing it, call save_artifact with ALL 8 files, then create_project_record.",
  ];
