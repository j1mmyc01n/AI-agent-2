...
import { Eye, ExternalLink, RefreshCw, Code2, Maximize2, Minimize2 } from "lucide-react";
...
function buildPreviewHtml(codeBlocks: CodeBlock[]): string | null {
  ...
  const htmlBlock = codeBlocks.find(
    (b) =>
      b.language === "html" &&
      (b.content.includes("<!DOCTYPE") || b.content.includes("<html") || b.content.includes("<body"))
  );
  if (htmlBlock) {
    // Inject any CSS/JS blocks into the HTML
    let html = htmlBlock.content;
    const cssBlocks = codeBlocks.filter((b) => b.language === "css" && b !== htmlBlock);
    const jsBlocks = codeBlocks.filter(
      (b) => (b.language === "javascript" || b.language === "js" || b.language === "typescript" || b.language === "ts" || b.language === "jsx" || b.language === "tsx") && b !== htmlBlock
    );
  ...
  const jsBlocks = codeBlocks.filter(
    (b) => b.language === "javascript" || b.language === "js" || b.language === "jsx"
  );
  const htmlSnippets = codeBlocks.filter(
    (b) => b.language === "html" && !b.content.includes("<!DOCTYPE") && !b.content.includes("<html")
  );

  // Only build if we have meaningful content
  if (cssBlocks.length === 0 && jsBlocks.length === 0 && htmlSnippets.length === 0) return null;
...
export default function PreviewPanel({ previewUrl, projectName, codeBlocks = [] }: PreviewPanelProps) {
  const [refreshKey, setRefreshKey] = useState(0);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const iframeRef = useRef<HTMLIFrameElement>(null);

  // Build inline preview from code blocks
  const inlineHtml = useMemo(() => buildPreviewHtml(codeBlocks), [codeBlocks]);

  // Determine what to show: deployed URL or inline preview
      ...
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-3 p-6">
        ...
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm opacity-70">
            Ask the AI to generate HTML, CSS, or JavaScript code and a live preview will appear here automatically.
            No GitHub or Netlify deployment needed!
          </p>
        </div>
      </div>
    );
